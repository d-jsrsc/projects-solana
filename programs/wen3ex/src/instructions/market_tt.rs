...
pub struct MarketTtCancel<'info> {
    ...
    #[account(
        ...
        // constraint = market_account.deposit_token_account == *deposit_token_account.to_account_info().key,
        close = creator
    )]
    pub market_account: Box<Account<'info, MarketTtAccount>>,
...
pub struct MarketTtExchange<'info> {
    ...
    #[account(
        ...
        // constraint = market_account.deposit_token_account == *creator_deposit_token_account.to_account_info().key,
        // constraint = market_account.receive_token_account == *creator_receive_token_account.to_account_info().key,
        constraint = market_account.creator == *creator.key,
        close = creator
    )]
    pub market_account: Box<Account<'info, MarketTtAccount>>,
